...
│   │   ├── harness.py     # run_turn — 오케스트레이터·서브에이전트 공통 턴 루프
...
- **병렬·백그라운드 서브에이전트 실행 없음** — 모든 위임은 순차적.
- **파일 편집 불필요** — 새 기능은 `@register_tool` 로 Python 함수를 추가하는 방식으로만 확장.
- 에이전트 페르소나는 `AGENTS/*.md`, 작업 가이드는 `SKILLS/*.md` 에 마크다운으로 정의.
