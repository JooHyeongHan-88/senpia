...
- 모듈/스코프 한정 상수(`STARTUP_GRACE`, `PRESENCE_*` 등)는 그대로 `backend/config.py`에 유지.
...
브라우저 → http://127.0.0.1:8765
  ├─ new EventSource('/api/presence?client_id=UUID') — 단일 SSE 채널이 생존 신호 역할
...
| `STARTUP_GRACE` | `60`초 | 최초 presence 연결 대기 상한 (초과 시 자동 종료) |
