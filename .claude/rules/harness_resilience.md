...
## Fallback Salvage 표현 (`backend/agent/harness/loop.py` + frontend, F6)
...
## 실패 턴 영속 + DoneEvent 보장 (`backend/agent/harness/loop.py`, R1)
...
## 반복 예산 임박 wind-down (`backend/agent/harness/loop.py` + `prompt/wind_down.py`, R7)
