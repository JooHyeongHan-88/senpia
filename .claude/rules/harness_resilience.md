...
## Pending 상태 자동 클리어 (`backend/agent/harness/loop.py`, F11)
...
## Fallback Salvage 표현 (`backend/agent/harness/lifecycle.py` + frontend, F6)
...
## 실패 턴 영속 + DoneEvent 보장 (`backend/agent/harness/loop.py` + `state/persistence.py`, R1)
...
## 루프 가드 파일 fingerprint (`backend/agent/harness/state/loop_guard.py`, R4)
...
## 턴 시작 terminal todo 리셋 (`backend/agent/harness/loop.py`, R5)
...
## 반복 예산 임박 wind-down (`backend/agent/harness/lifecycle.py` + `prompt/wind_down.py`, R7)
...
## pending 동안 SKILL 본문 캐리 (`backend/agent/harness/loop.py`, R9)

SKILL 은 stateless 트리거 라우터라 매 턴 `skill_registry.select(user_message)` 가 최신
메시지로 재매칭한다. ask_user(슬롯 가드·sentinel)·서브에이전트 슬롯으로 턴이 끊겼다가
사용자가 답하면, 그 답변 메시지("2024-01-01" 등)엔 트리거가 없어 `select()=[]` → 직전
SKILL 본문이 시스템 프롬프트에서 증발했다(정작 절차 지침이 가장 필요한 순간).

- `run_turn` 의 select 직후, `state/pending.has_pending(state)` 가 참이고 `state.active_skills`
  가 비어있지 않으면 그 이름들을 `skill_registry.get_by_names` 로 다시 로드해 select 결과에
  병합한다(중복 제외). 캐리된 SKILL 은 `skills` 리스트를 타고 그대로 `compose` 의 `# Skill:`
  본문 주입을 받으므로 **composer 무변경**.
- **pending-gated = 영구 sticky 아님**: pending 이 해소되는 다음 턴(F11 클리어)부터는 캐리되지
  않아 SKILL 이 정상 드롭된다 → R5/F11 의 "공격적 클리어" 철학과 충돌하지 않는다.
- `has_pending` 은 `clear_all_pending` 과 **동일한 필드 묶음**(pending_tool / missing_slots /
  pending_question / pending_sub_agent)을 본다 (SSOT). 한쪽만 바꾸지 말 것.
- 부수효과(무해): `SkillActiveEvent` 가 캐리된 SKILL 로 재발화 → 프론트 뱃지가 답변 턴에 다시
  표시(더 정직한 UX). 프론트 변경 없음.

테스트: `backend/tests/test_harness_skill_carry.py`

---

## summarize-then-drop 롤링 압축 (`backend/agent/harness/compaction.py` + `stores/conversation.py`, R10)

LLM 컨텍스트는 `MAX_HISTORY_MESSAGES`(40) 슬라이딩 윈도우라 넘치면 **앞에서 그냥 삭제**됐다
(요약 없음) — 긴 세션/나중 재개 시 원래 목표·중간 결정·제약이 LLM 시야에서 증발했다.
todo/Session Artifacts 는 "단계·산출물"은 보존하나 **서사·의도·결정**은 못 잡는다.

- **objective 앵커**: `run_turn` 이 세션 첫 턴(history 비어있음)의 user_message 를
  `state.objective` 에 1회 박제한다(이후 불변, `OBJECTIVE_MAX_CHARS` 상한). 무비용.
- **store 가 버린 것을 반환**: `ConversationStore.append()` 가 트림으로 버린 메시지 리스트를
  반환한다(cut+tool-pair 경계 로직은 한 곳 유지). store 는 provider 무접근 순수 저장소라 요약은
  하니스 책임 — append 직후 `_compact_history(provider, prior, dropped)` 가 잘린 턴을 기존
  요약에 접어 `state.progress_summary`(롤링 단일 문자열, 무한 성장 없음)에 갱신한다.
- **best-effort**: 요약 콜이 실패하면 직전 요약 유지(graceful degrade — content_sync 철학).
  `APP_COMPACTION_ENABLED`(기본 true)로 토글. **happy path 전용** — 예외 경로
  (`_persist_failed_turn`)엔 LLM 콜을 추가하지 않는다(드물고, 실패 경로 부담 회피).
- **표면화**: `_render_progress_summary_section(state)`(sections.py)가 objective+progress_summary 를
  `# 이전 진행 요약` 으로 주입(두 composer 의 To-do 섹션 직전). 마지막 줄은 namespace 휘발 대비
  `load_artifact` 복원 너지다(휘발 변수는 save→load 가 정식 경로).
- **저장 위치 결정**: 요약은 `_artifacts.jsonl` manifest 가 아니라 **AgentState 필드**에 둔다 —
  산출물이 아니라 대화 상태라 manifest(parquet 중심 재발견)를 오염시키지 않는다.

테스트: `backend/tests/test_harness_compaction.py` · `backend/tests/test_conversation_store.py`
(append dropped 반환) · `backend/tests/test_prompt_sections.py`
