...
- 액센트 틴트: `--accent-soft`(10%)·`--accent-soft-strong`(18%)·`--accent-border`(30%) — `:root` 1회 선언으로 양 테마 대응 (같은 html 요소에서 `--accent` 캐스케이드 재해석). 컴포넌트에서 일회성 `color-mix(accent N%)` 작성 금지, 이 토큰을 쓸 것
