...
# 기동 시 잔존 .old 스윕용 재시도 파라미터. Updater 가 정상 정리하면 대상이 없어
# 즉시 끝나고, 드물게 Updater 가 중간에 죽어 남긴 orphan 만 청소한다.
_BACKUP_DELETE_RETRY = 20
_BACKUP_DELETE_INTERVAL = 0.5
...
def _notify_shell(folder: Path) -> None:
    ...
    Updater 의 notify_shell 과 동일 역할이나, updater 모듈은 backend 를 import 할 수
    없으므로 헬퍼 공유가 불가능해 의도적으로 소량 중복한다.
    """
    ...
    try:
        ...
        logger.debug("notify_shell failed: %s", e)


def cleanup_stale_backup() -> None:
    """기동 시 형제 {stem}.old 백업을 best-effort 로 청소한다 (안전망).

    정상 흐름에서는 Updater 가 교체 직후 .old 를 지우지만, Updater 가 중간에 죽으면
    orphan .old 가 남아 누적된다. 새 EXE 는 Updater 보다 오래 살아 잠금이 풀린 뒤
    삭제할 여유가 있으므로 데몬 스레드에서 재시도 삭제 + 탐색기 새로고침한다.

    frozen EXE 에서만 의미가 있다(dev 는 sys.executable 이 python.exe).
    """
    ...
    for _ in range(_BACKUP_DELETE_RETRY):
        try:
            backup.unlink()
            break
            ...
            time.sleep(_BACKUP_DELETE_INTERVAL)
    else:
        logger.debug("cleanup_stale_backup: %s still locked, leaving as-is", backup)
        return

    _notify_shell(current.parent)
