...
# 다중 선택 변형 — 단일 B 트리거보다 먼저 검사해야 한다("여러개 골라줘"가 "골라줘" 포함).
_B_MULTI_TRIGGERS = ("여러개 골라줘", "모두 선택", "복수 선택", "pick multiple")
...
class MockProvider:
    ...
    async def astream(
        ...
        # 다중 선택 변형 — 단일 트리거보다 먼저 검사("여러개 골라줘" ⊃ "골라줘").
        if last_user is not None and _matches(last_user.content, _B_MULTI_TRIGGERS):
            async for event in _scenario_B_ask_user(
                last_user.content, multi_select=True
            ):
                yield event
            return
...
async def _scenario_B_ask_user(
    user_text: str, *, multi_select: bool = False
) -> AsyncIterator[StreamEvent]:
    """ReasoningEvent 청크 → ask_user(both 모드) 호출 → Done.

    multi_select=True 면 복수 선택 변형으로 질문/옵션을 바꾸고 다중 선택 카드를 띄운다.
    답변 후속(echo)은 `mock-B-ask-` id 접두를 공유해 단일/다중이 동일 경로를 탄다.
    """
    if multi_select:
        reasoning = (
            f"사용자가 '{user_text}' 라고 했습니다. "
            "여러 항목을 동시에 고를 수 있도록 multi_select 로 ask_user 를 호출합니다..."
        )
    else:
        reasoning = (
            f"사용자가 '{user_text}' 라고 했습니다. "
    ...
    if multi_select:
        arguments = {
            "question": "리포트에 포함할 항목을 모두 골라주세요.",
            "options": [
                "매출 추이",
                "지역별 분포",
                "카테고리 비중",
