...
_SOURCE_KINDS: frozenset[str] = frozenset({"parquet"})
    ...
    if kind in _CONTENT_KINDS:
        ...
        if source is not None:
            return (
                ...
                "namespace 변수 저장은 kind='parquet' 에서만 가능합니다."
            )
        return None
...
@register_tool(
    description=(
        "마크다운/JSON/텍스트/parquet 산출물을 result/<session>/<ts>/<filename> 경로에 저장한다. "
        "When to use: LLM 이 작성한 보고서·분석 데이터·DataFrame 을 사용자에게 영속적으로 "
        "남기고 싶을 때. parquet 은 차트 데이터 직렬화 용 (display_chart 의 spec 이 참조). "
        ...
        "parquet 은 source 필수 (namespace 변수 참조 '$varname'). 둘은 상호 배타적. "
        "filename 은 슬래시·역슬래시·'..' 없는 단순 파일명만 허용하며, kind 와 확장자가 일치해야 한다 "
        "(markdown↔.md, json↔.json, text↔.txt, parquet↔.parquet)."
    ),
    slot_prompts={
...
async def save_artifact(
    ...
    kind: Annotated[
        Literal["markdown", "json", "text", "parquet"],
