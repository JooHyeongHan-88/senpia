...
backend/
  ...
  core/               앱 인프라 (LLM 무관)
    ...
    result_store.py   산출물 저장 유틸리티 — contextvars 기반 세션 메타 전달 + artifact_slot()
  ...
  agent/              LLM 에이전트 런타임
    ...
    tools/            @register_tool 데코레이터 기반 사내 API 도구 모음
      ...
      visualize.py    display_image / display_chart / display_markdown (아티팩트 패널 도구)
...
docs/                 개발자 참고 문서. builtin-tools.md · mock-scenarios.md · skills.md · agents.md 등
...
**에이전트 산출물 경로** (`result/` — .gitignore 대상)
```
result/{세션제목}-{client_id[:8]}/        ← session_dir_name()
  {YYYYMMDD-HHmmss}/                      ← artifact_slot() 호출마다 새 슬롯 생성
    favicon.svg / correlation.json / report.md ...
```
- `core/result_store.py` 의 `set_session_context(client_id, session_title)` 를 harness 가 매 턴 시작 시 호출.
- 도구·Mock Provider 는 `artifact_slot()` 을 호출해 경로를 얻고 `display_image` / `display_markdown` 에 `result/...` 경로로 전달.
- `session_title` 은 프론트엔드가 `/api/chat?session_title=...` query param 으로 전달 → harness → `set_session_context`.
