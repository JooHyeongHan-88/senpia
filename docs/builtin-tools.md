...
ECharts 인터랙티브 차트(들)를 아티팩트 패널에 표시한다. **한 번의 호출에 여러 차트를
list 로 전달**하면 패널이 반응형 그리드(최대 6개/페이지) + prev/next 페이지네이션으로
자동 렌더링한다. 단일 차트도 1-element list 로 전달한다.
드래그 선택(brush), 확대(dataZoom), 저장(saveAsImage) 도구가 각 차트에 자동 포함된다.

#### 인자

| 인자 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `charts` | `list[ChartItem]` | **필수** | 차트 항목 리스트. 빈 list 면 에러 반환 |
...
| `series` | `list[{name, data}]` | 조건부 | `null` | 시리즈 목록. `option` 지정 시 무시됨 |
| `x_label` | string | 선택 | `""` | X축 레이블 |
| `y_label` | string | 선택 | `""` | Y축 레이블 |
| `extra_option` | object | 선택 | `null` | ECharts option 추가 필드 (deep-merge, list 는 교체) |
...
1. 각 `charts[i]` 의 `chart_type` + `series` 로 ECharts `option` JSON 을 자동 생성하거나, `option` 이 있으면 그대로 채택
2. `extra_option` 을 깊은 병합 (list 는 교체)
3. `ToolResult.data = {kind:"chart", items:[{chart_type, title, option}, ...]}` 반환
4. 한 항목이라도 series·option 둘 다 비어 있으면 전체 호출이 `is_error=True` 로 실패
5. 프론트엔드: 패널 반응형 그리드(`auto-fit minmax(320px)`), 페이지당 최대 6개, 페이지 전환 시 ECharts 인스턴스 dispose+init. 셀 클릭 시 라이트박스 모달
6. 메시지 버블에 "📊 차트 N개" 칩 추가
...
```json
{
  ...
  "arguments": {
    "charts": [
      {"chart_type": "bar",     "title": "월별 매출", "series": [...]},
      {"chart_type": "line",    "title": "주가 추세", "series": [...]},
      {"chart_type": "scatter", "title": "상관 분석", "series": [...]}
    ]
  }
}
```
...
**ECDF** — ECharts 네이티브 타입 없음. data 는 `[[값, 누적확률], ...]` scatter 포맷.
    ...
    "charts": [
      {
        ...
        "title": "누적분포함수 (ECDF)",
        "series": [
          { "name": "ECDF", "data": [[1.0, 0.1], [2.1, 0.2], [3.4, 0.5], [4.8, 0.8], [6.2, 1.0]] }
        ],
        "x_label": "값",
        "y_label": "누적 확률",
        "extra_option": {
...
**Box Plot** — `chart_type="box"`. data 각 항목은 `[min, Q1, median, Q3, max]`.
    ...
    "charts": [
      {
        ...
        "extra_option": { "xAxis": { "data": ["그룹A", "그룹B"] } }
      }
    ]
  }
}
    ...
    "charts": [
      {
        "option": {
          ...
          "series": [
            { "type": "radar", "data": [{ "name": "현재", "value": [80, 95, 70] }] }
          ]
        }
      }
    ]
...
4. `display_chart(charts=[{chart_type, series, title, ...}, ...])` 로 결과 시각화 → `complete_todo`
```
...
```markdown
## 시각화 규칙
- 시계열 scatter: 각 ChartItem 에 `chart_type="scatter"`, `extra_option={"xAxis": {"type": "time"}}` 필수.
...
- 여러 차트를 한 번에 보고 비교하려면 charts 리스트에 모두 담아 단일 호출로 전달.
```
