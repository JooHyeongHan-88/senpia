...
2. spec 을 `ChartSpecV1` 로 파싱·검증 (실패 시 첫 오류 위치 회신). histogram 의 `x.bin` 누락은 validator 가 자동 보정
3. `render_spec_to_echarts()` 가 각 차트의 parquet 를 읽어 ECharts option 리스트 생성 (mark·encoding 비호환 시 에러). `data.source` 파일 미발견 시 세션 manifest 의 실존 parquet 후보를 에러 메시지에 함께 안내 — LLM 이 경로 추측 없이 self-correct
