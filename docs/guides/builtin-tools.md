...
2. spec 을 `ChartSpecV1` 로 파싱·검증 (실패 시 위치 포함 오류 최대 3건 회신). histogram 의 `x.bin` 누락은 validator 가 자동 보정. `type:"normal"`→`nominal`, `mark:"hist"`→`histogram`, `aggregate:"avg"`→`mean` 같은 근사 표기도 자동 정규화 (대소문자 무관)
3. `render_spec_to_echarts()` 가 각 차트의 parquet 를 읽어 ECharts option 리스트 생성 (mark·encoding 비호환 시 에러). `data.source` 파일 미발견 시 세션 manifest 의 실존 parquet 후보를 에러 메시지에 함께 안내 — LLM 이 경로 추측 없이 self-correct. `extra_option.legend.data` 가 실제 시리즈 이름과 어긋나면 실존 이름으로 자동 보정 (레전드 소실 방지)
...
4. 프론트엔드가 우측 패널에 확장 SPA 를 **iframe 으로 임베드**(`ArtifactExtension`)하고 자동으로 연다.
   패널 헤더의 **'최대화'** 버튼으로 본문을 화면 전체로 키울 수 있다. 칩은 메시지에 영속되어 세션 재진입
   후에도 같은 번들로 복원된다.
...
| `multi_select` | bool | 선택 | `false` | `true` 면 옵션 다중 선택 후 한 번에 제출 |
...
복수 선택이 자연스러운 질문(여러 항목을 동시에 고르는 게 타당)일 때 `true` 로 호출한다
(예: "리포트에 포함할 항목을 모두 고르세요"). `AskUserCard` 가 옵션을 **토글 칩**으로 그리고
**"선택 완료" 버튼**을 띄우며, 사용자가 고른 항목들은 `", "` 로 합쳐진 한 메시지로 제출된다.
`options` 가 없으면(자유 입력 전용) `multi_select` 는 무시된다(`false` 로 강제). 기본은
`false`(단일 선택 — 버튼 클릭 즉시 제출).
