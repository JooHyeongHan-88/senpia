...
analyst_agent sub (5턴 — parquet + spec 파이프라인)
  턴 1  → add_todo(3)
  턴 2  → exec_code(samples_df·corr_df·grouped_df) + call_function(stats_df) + complete_todo(1)
  턴 3  → save_artifact(parquet) × 4 (samples/stats/corr/grouped) + complete_todo(2)
  턴 4  → save_artifact(json, charts.spec.json) + display_chart(7 차트) + complete_todo(3)
  턴 5  → complete_subagent(summary)
...
writer_agent sub
  ...
  턴 2  → display_image(images=[10장 SVG data URI 색상 카드])
  턴 3  → complete_subagent(summary)
...
E 시나리오는 차트 7개 list → 페이지네이션(1페이지 6 + 2페이지 1) + 이미지 10장 list →
무한 스크롤 갤러리 + N/M 카운터를 동시에 검증한다. 이미지 항목은 외부 파일 의존이 없도록
색상 카드 SVG data URI 로 인라인 생성된다.
...
### G. artifact 재발견·재사용 (읽기 방향 체인)

```
오케스트레이터 (sub-agent 없이 직접 실행)
  턴 1  → ToolCallEvent(list_artifacts, kind="parquet")
  턴 2  → list 결과에서 첫 'result/...parquet' 경로 추출
        → ToolCallEvent(load_artifact, path=<추출경로>, store_as="reloaded_df")
  턴 3  → ToolCallEvent(exec_code, code="reloaded_df.shape ... print(...)")
  턴 4  → DeltaEvent(재로드 완료 보고 + reloaded_df 안내)
```

보통 **D 를 먼저 실행**해 `samples.parquet`/`stats.parquet` 가 디스크에 있는 상태를 전제한다.
`_extract_first_parquet_path` 가 list 결과 텍스트에서 경로를 정규식으로 뽑는데, 세션 폴더명에
공백이 있을 수 있어(`데이터 요약-...`) `\S+` 가 아니라 `result/[^\n]+?\.parquet` 로 매칭한다.
parquet 산출물이 없으면 "먼저 분석을 실행하라" 는 안내로 graceful 종료한다.

검증 포인트: `list_artifacts`/`load_artifact`/`exec_code` 3개 도구 카드가 순차 완료되고,
최종 응답에 재로드 경로와 `reloaded_df` shape 가 인용되는지 — artifact 의 **읽기 방향**
(디스크 → namespace) 이 frozen 안전 경로로 동작함을 확인한다.
...
`data.kind="extension"` 칩(`{tool, src:"/ext/evaluator/?bundle=...", title}`)을 반환한다 —
프론트는 이 칩을 **우측 패널에 확장 SPA 를 iframe 으로 임베드**하는 `ArtifactExtension`
컴포넌트로 렌더하고 자동으로 연다(`chatActions.svelte.js` 의 `_ARTIFACT_TOOL_NAMES`·
